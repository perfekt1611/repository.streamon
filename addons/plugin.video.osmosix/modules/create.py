...
def initialize_DialogBG(mess1,mess2, barType="BG"):
    if barType == "BG":
        if not thisDialog.dialogeBG:
            ...
            thisDialog.dialogeBG.create("osmosix: " + mess1 + ": " , " " + mess2)
        
    else: 
        if not thisDialog.dialoge:
            thisDialog.dialoge = xbmcgui.DialogProgress()
            thisDialog.dialoge.create("osmosix: " + mess1 + ": " , " " + mess2)
...
def fillPlugins(cType='video'):    
    ...
    for addon in sorted(json_details["addons"], key=lambda json: json['name'].lower()):
        ...
        if cType in addontypes and not addon['addonid'] == 'plugin.video.osmosix':
    ...
    if strm_type.find('Cinema') != -1 or strm_type.find('YouTube') != -1 or strm_type.find('Movies') != -1:
        try:
            ...
            for i in dbMovList:
                ...
                fileSys.writeSTRM(stringUtils.cleanStrms(i[0].rstrip(".")), stringUtils.cleanStrms((i[1].rstrip("."))) , "plugin://plugin.video.osmosix/?url=plugin&mode=10&mediaType=movie&id=" + str(i[2]) + "|" + i[1])
...
def getEpisode(episode_item, strm_name, strm_type, j=0, pagesDone=0):
    ...
    for i in dbEpisode:        
        fileSys.writeSTRM(os.path.join(stringUtils.cleanStrms((i[0].rstrip("."))),stringUtils.cleanStrms(i[1].rstrip("."))), stringUtils.cleanStrms(i[3] + i[4]) , "plugin://plugin.video.osmosix/?url=plugin&mode=10&mediaType=show&episode=" + i[3] + i[4] + "&showid=" + str(i[2]) + "|" + i[1])
    return pagesDone
