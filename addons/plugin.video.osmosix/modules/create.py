...
def getEpisode(episode_item, strm_name, strm_type, j=0, pagesDone=0):
    ...
    if dbEpisode is not None:
        fileSys.writeSTRM(dbEpisode.get('path'), dbEpisode.get('strSeasonEpisode'), "plugin://plugin.video.osmosix/?url=plugin&mode=10&mediaType=show&episode=" + dbEpisode.get('strSeasonEpisode') + "&showid=" + str(dbEpisode.get('showID')) + "|" + dbEpisode.get('tvShowTitle'))
    return pagesDone
